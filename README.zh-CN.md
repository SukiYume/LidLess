...
合盖行为高度依赖具体机型、固件、Windows 电源栈和厂商工具。下面是目前真实验证过的
机器/配置：

| 笔记本 / 型号 | Windows build | PowerShell | 结果 |
|---------------|---------------|------------|------|
| ASUS ROG Zephyrus G14 GA403UV | Windows 11 10.0.26200 (build 26200) | 5.1.26100.8457 / 7.6.2 | 通过：AC 供电时，真实使用中合盖 agent 任务保持唤醒并可联网。 |
...
- `start` 需要安装目录仅管理员可写，例如 `%ProgramFiles%\LidLess`。不要从桌面、
  下载目录或其他普通用户可写目录注册后台任务。

## 安装

先右键以管理员身份打开 PowerShell，然后整块复制下面命令，粘贴后回车：
...
如果脚本或 `src\` 模块能被普通用户写入，`start` 会拒绝注册 `SYSTEM` 任务。启动成功后，
它会注册并启动后台任务。之后只要配置的 agent 在运行，你就可以合盖。想停止并完整
还原电源设置时，在管理员 PowerShell 中运行：
...
前台 `run`/`once` 会和后台 `SYSTEM` 任务共用 `state/state.json`，因此只建议用于
诊断；除非正在排查问题，否则不要和已安装的后台任务并发运行。
...
- 某些 OEM 固件或厂商工具可能无视 Windows 电源策略强制睡眠。遇到这种情况时，
  LidLess 不能仅靠用户态 PowerShell 覆盖固件行为。
- 企业组策略或设备管理工具可能在 LidLess 运行期间覆盖电源设置。
- 如果机器被固件强制进入 Modern Standby，网络仍可能断开。LidLess 的策略是在保护
  生效时阻止进入 standby，而不是保证 standby 内的网络连接可靠。
- `powercfg` 解析优先读取注册表，并以 `powercfg` 输出末尾的十六进制值兜底。极少数
  非常规 Windows 构建或本地化输出可能不符合这个假设。
...
- **合盖后还是睡了。** 运行 `.\LidLess.ps1 doctor`，确认匹配的进程出现在
  ...
  显示为 `0 (Do nothing)`。`doctor` 里的电源事件有助于判断是什么触发的。
